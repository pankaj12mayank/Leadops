...
SELECTORS = {
    "cookie_accept": "button:has-text('Accept'), button:has-text('Got it'), button:has-text('Allow All'), button:has-text('Agree')",
    "result_cards": (
...
def _build_search_url(query: str) -> str:
    ...
    return f"https://clutch.co/search?q={q}"
...
async def _safe_extract_attribute(page: Page, card, selector: str, attr: str, logger, timeout: int = 2000) -> Optional[str]:
    for single_sel in selector.split(", "):
        try:
            el = card.locator(single_sel).first
            if await el.count() > 0 and await el.is_visible(timeout=timeout):
...
async def _extract_employees(text: str) -> Optional[str]:
    if not text:
        return None
    text = text.strip()
    match = re.search(r"(\d[\d,]*\s*-\s*\d[\d,]*)\s*(Employees?|people)", text, re.IGNORECASE)
    if match:
                ...
                for i in range(count):
                    try:
                        ...
                        if data is None:
                            ...
                            continue
                        dup_key = data["clutch_profile_url"] or data["company_name"]
...
_export_logger = logging.getLogger("scrapers.clutch")


async def _export_clutch_results(data: List[Dict[str, Any]], query: str, cfg: Dict[str, Any]) -> Optional[Path]:
    if not data:
        return None
    ...
    filename = f"{ts}_clutch_{safe_query}"
    ...
    if ext not in known:
        ...
        ext = "csv"
    out = BASE_DIR / "exports" / "clutch" / f"{filename}.{ext}"
    ...
    try:
        if ext == "csv":
            ...
            try:
                ...
                _export_logger.error("XLSX export failed for clutch: %s, falling back to CSV", xl_err)
                ext = "csv"
                out = out.with_suffix(".csv")
                df.to_csv(out, index=False, encoding="utf-8-sig")
        return out
    except Exception as e:
        _export_logger.error("Export failed for clutch: %s", e)
        return None
...
async def run_clutch_scraper(context: BrowserContext, logger, cfg: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if all_leads:
            export_path = await _export_clutch_results(all_leads, query, cfg)
            if export_path:
