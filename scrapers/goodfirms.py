...
async def run_goodfirms_scraper(
    ...
    try:
        ...
        if all_leads:
            export_path = export_dataframe_to_file(
                pd.DataFrame(all_leads), f"goodfirms_{query[:50]}", "goodfirms", cfg, logger
            )
            if export_path:
        ...
        if all_leads:
            export_dataframe_to_file(pd.DataFrame(all_leads), f"goodfirms_{query[:50]}", "goodfirms", cfg, logger)
        ...
        if page:
            try:
                ...
                logger.debug("Failed to close GoodFirms page")
