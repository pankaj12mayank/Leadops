...
def _get_export_base() -> Path:
    ...
    if _EXPORT_BASE is None:
        try:
            cfg_path = BASE_DIR / "backend" / "config" / "config.json"
            if cfg_path.exists():
                with open(cfg_path, encoding="utf-8") as f:
                    cfg = json.load(f)
                exports_rel = cfg.get("paths", {}).get("exports", "content")
                _EXPORT_BASE = (BASE_DIR / exports_rel).resolve()
            else:
                _EXPORT_BASE = BASE_DIR / "content"
...
def cleanup_old_exports(logger: logging.Logger, retention_days: int = 30, max_size_mb: int = 500) -> int:
    export_root = BASE_DIR / "content"
    if not export_root.exists():
        logger.info("No content directory found, skipping cleanup")
        return 0
