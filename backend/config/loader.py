...
CONFIG_PATH = BASE_DIR / "backend" / "config" / "config.json"
CONFIG_BACKUP_PATH = BASE_DIR / "backend" / "config" / "config.backup.json"
...
REQUIRED_DIRS = [
    BASE_DIR / "storage",
    BASE_DIR / "content" / "agency",
    BASE_DIR / "content" / "startup",
...
def setup_logging(name: str = "lead_system") -> logging.Logger:
    ...
    log_file_name = LOG_FILE_MAP.get(name, log_cfg.get("file", "logs/system.log") if name == "lead_system" else f"logs/{name}.log")
